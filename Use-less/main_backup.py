...
import sys
import time
from gpiozero import LED # Changed from RPi.GPIO
...
# --- GPIO SETUP ---
# Initialize the LED using gpiozero
led_status = LED(LED_PIN)
led_status.off()  # Default OFF
...
def signal_handler(sig, frame):
    ...
    # Turn off the LED before exiting (gpiozero handles cleanup automatically)
...
try:
    ...
    try:
        ...
        led_status.on()  # Success: LED ON (using gpiozero)
...
# --- Critical Error Handling Loop ---
    ...
    while True:
        try:
            led_status.off() # Turn off LED (using gpiozero)
            lcd.clear()
            lcd.cursor_pos = (0, 0)
